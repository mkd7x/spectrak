...
`GET /health` executes a lightweight SQLite query and returns `{ "status": "ok" }` when the service and database are ready. A database failure returns HTTP 503 with `ERR_HEALTHCHECK_FAILED`.
